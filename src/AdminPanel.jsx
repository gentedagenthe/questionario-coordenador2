...
export default function AdminPanel() {
  ...
  const carregar = async () => {
    const { data } = await supabase.from('candidatos_dei2').select('*').order('criado_em', { ascending: false });
  ...
  const salvarObs = async () => {
    setSalvando(true);
    await supabase.from('candidatos_dei2').update({ observacoes: obs }).eq('id', selecionado.id);
  ...
  const alterarStatus = async (id, status) => {
    await supabase.from('candidatos_dei2').update({ status }).eq('id', id);
      ...
      {selecionado && (
        <div style={s.modal} onClick={fechar}>
          <div style={s.modalCard} onClick={e => e.stopPropagation()}>
            ...
            {campo('Disponibilidade para início', selecionado.disponibilidade_inicio)}
            {campo('Disponibilidade para viagens', selecionado.disponibilidade_viagens)}
