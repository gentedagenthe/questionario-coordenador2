      ...
      <div style={styles.card}>
        ...
        {etapa === 1 && (
          <>
            ...
            <label style={styles.label}>Você possui disponibilidade para viagens? *</label>
            <div style={styles.radioGroup}>
              {['Sim, frequentemente', 'Sim, às vezes', 'Não'].map(op => (
                <div key={op} style={styles.radioItem(form.disponibilidade_viagens === op)} onClick={() => set('disponibilidade_viagens', op)}>
                  <span>{form.disponibilidade_viagens === op ? '🔵' : '⚪'}</span> {op}
                </div>
              ))}
            </div>
          </>
